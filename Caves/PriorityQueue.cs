...
namespace Caves
{
    ...
    /// Modified by Kaci Y to compare by total cost of cave 
    /// </summary>
    public class PriorityQueue<T>
    where T:Cave
    {
        List<Cave> storedValues;

        public PriorityQueue()
        {
            //Initialize the array that will hold the values
            storedValues = new List<Cave>();

            //Fill the first cell in the array with an empty value
            storedValues.Add(default(Cave));
        }

        //copied over from List
        public bool Contains(Cave item)
        {
            if (this.Count != 0)
                return storedValues.IndexOf(item) != -1;
            return false;
        }
        ...
        /// </summary>
        public virtual Cave Peek()
        {
            if (this.Count == 0)
                return default(Cave); //Priority Queue empty
            else
        ...
        /// </summary>
        public virtual void Enqueue(Cave value)
        {
        ...
        protected virtual void BubbleUp(int startCell)
        {
            ...
            while (this.IsParentBigger(cell))
            {
                ...
                Cave parentValue = this.storedValues[cell / 2];
                Cave childValue = this.storedValues[cell];
        ...
        protected virtual void BubbleDown(int startCell)
        {
            ...
            while (this.IsLeftChildSmaller(cell) || this.IsRightChildSmaller(cell))
            {
                ...
                {
                    //Swap values
                    Cave parentValue = storedValues[cell];
                    Cave leftChildValue = storedValues[2 * cell];
                ...
                else if (child == 1) //Right Child
                {
                    //Swap values
                    Cave parentValue = storedValues[cell];
                    Cave rightChildValue = storedValues[2 * cell + 1];
        ...
        protected virtual bool IsParentBigger(int childCell)
        {
            ...
            else
            {
                ...
                return storedValues[childCell / 2].TotalCost.CompareTo(storedValues[childCell].TotalCost) > 0;
        ...
        protected virtual bool IsLeftChildSmaller(int parentCell)
        {
            if (2 * parentCell >= storedValues.Count)
                ...
                return storedValues[2 * parentCell].TotalCost.CompareTo(storedValues[parentCell].TotalCost) < 0;
        ...
        protected virtual bool IsRightChildSmaller(int parentCell)
        {
            if (2 * parentCell + 1 >= storedValues.Count)
                ...
                return storedValues[2 * parentCell + 1].TotalCost.CompareTo(storedValues[parentCell].TotalCost) < 0;
        ...
        protected virtual int CompareChild(int parentCell)
        {
            ...
            if (leftChildSmaller || rightChildSmaller)
            {
                if (leftChildSmaller && rightChildSmaller)
                {
                    ...
                    Cave leftValue = this.storedValues[leftChild];
                    Cave rightValue = this.storedValues[rightChild];

                    //Compare the values of the children
                    if (leftValue.TotalCost.CompareTo(rightValue.TotalCost) <= 0)
